...
## Required Files
- `snake_game.py` (The Client/Victim app)
...
2. **Configure and Build the Client**
   - **Set the IP:** Open `snake_game.py` and update the `server_ip` variable with your machine's IPv4 address.
   - **Install PyInstaller:**
     ```bash
...
3. **Deployment**
   - Transfer the `snake_game.exe` from the `dist` folder to the victim's machine.
